...
def main():
    ...
    # 初始化模型 (Signal_MAE_RoPE)
    # 构造 base_model_config
    base_model_config = {
        'signal_len': config['data']['signal_len'],
        'patch_size': config['model'].get('patch_size', 50),
    ...
    if is_main_process():
        logger.info("Initializing Signal_MAE_RoPE...")

    model = Signal_MAE_RoPE(**base_model_config)
    model.to(device)
