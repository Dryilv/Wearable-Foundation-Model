        ...
        for fp in file_paths:
            try:
                with open(fp, 'rb') as f:
                    ...
                    # 适配 v2 的数据结构 (通常是单通道或扁平化)
                    raw_data = content['data'] if isinstance(content, dict) else content
                    if raw_data.ndim == 2: raw_data = raw_data[0] # 取第一行
...
def main():
    ...
    parser.add_argument('--output_csv', type=str, default="inference_report_v2.csv")
    ...
    # CoT 参数
    parser.add_argument('--use_cot', action='store_true', help="是否使用 CoT Head")
    ...
    model = TF_MAE_Classifier(
        ...
        use_cot=args.use_cot # v2 支持可选 CoT
    )
