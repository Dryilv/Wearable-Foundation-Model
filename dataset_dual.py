...
class DualChannelDataset(Dataset):
    def __init__(self, data_root, split_file, mode='train', signal_len=3000, task_index=0, num_classes=2):
        ...
        with open(split_file, 'r') as f:
            splits = json.load(f)
        self.file_list = splits[mode]
        print(f"[{mode}] Loaded {len(self.file_list)} ECG samples (use channel 1 only).")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 4. 转成 Tensor（单通道）
            signal_tensor = torch.from_numpy(norm_ecg)  # shape: [signal_len]
