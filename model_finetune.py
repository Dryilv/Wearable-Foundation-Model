...
class TF_MAE_Classifier(nn.Module):
    def __init__(self, pretrained_path, num_classes, hidden_dim=512, dropout_rate=0.5, 
        ...
        # 4. 分类头
        # 输入是 GAP 后的特征 (embed_dim)，不再依赖 CLS token
    ...
    def forward(self, x):
        ...
        # Index 0 是 CLS token，Index 1: 是 Patch tokens
        latent, _, _ = self.encoder_model.forward_encoder(imgs)
        
        # 4. Classification Head (Global Average Pooling)
        # 【关键修改】: 丢弃 CLS token，使用所有 Patch Token 的均值
