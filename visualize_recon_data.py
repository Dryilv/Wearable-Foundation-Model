import os
import argparse
import yaml
import torch
import matplotlib.pyplot as plt
import numpy as np
import pickle
from torch.amp import autocast

# 导入你的模型定义
# 确保 model_rope_complete.py 在同一目录下
from model import CWT_MAE_RoPE, cwt_wrap

def load_model(config_path, checkpoint_path, device):
    """加载配置和模型权重"""
    print(f"Loading config from {config_path}...")
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)

    print(f"Initializing model...")
    # 实例化模型 (注意参数要与训练时一致)
    model = CWT_MAE_RoPE(
        signal_len=config['data']['signal_len'],
        cwt_scales=config['model'].get('cwt_scales', 64),
        patch_size_time=config['model'].get('patch_size_time', 50),
        patch_size_freq=config['model'].get('patch_size_freq', 4),
        embed_dim=config['model']['embed_dim'],
        depth=config['model']['depth'],
        num_heads=config['model']['num_heads'],
        decoder_embed_dim=config['model']['decoder_embed_dim'],
        decoder_depth=config['model']['decoder_depth'],
        decoder_num_heads=config['model']['decoder_num_heads'],
        mask_ratio=config['model']['mask_ratio'], # 可视化时这个参数主要影响随机Mask的生成
        mlp_rank_ratio=config['model'].get('mlp_rank_ratio', 0.5), # 关键：必须匹配
        time_loss_weight=config['model'].get('time_loss_weight', 1.0)
    )
    
    if os.path.isfile(checkpoint_path):
        print(f"Loading weights from {checkpoint_path}...")
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        
        # 处理权重键名 (去除 DDP 和 Compile 前缀)
        state_dict = checkpoint['model'] if 'model' in checkpoint else checkpoint
        new_state_dict = {}
        for k, v in state_dict.items():
            name = k
            if name.startswith('module.'): name = name[7:]
            if name.startswith('_orig_mod.'): name = name[10:]
            new_state_dict[name] = v
            
        model.load_state_dict(new_state_dict)
        print("Weights loaded successfully.")
    else:
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")

    model.to(device)
    model.eval()
    return model, config

def preprocess_signal(raw_signal, target_len=3000):
    """
    对新信号进行预处理，使其符合模型输入要求
    1. 转 float32
    2. Z-Score 归一化 (关键!)
    3. 裁剪或填充到 target_len
    """
    signal = raw_signal.astype(np.float32)
    
    # 1. Z-Score 归一化
    # 训练时 Dataset 做了归一化，推理时必须保持一致，否则 CWT 幅度会不对
    mean = np.mean(signal)
    std = np.std(signal)
    if std < 1e-6: std = 1e-6
    signal = (signal - mean) / std
    
    # 2. 长度处理
    current_len = len(signal)
    if current_len > target_len:
        # 取中间段
        start = (current_len - target_len) // 2
        signal = signal[start : start + target_len]
    elif current_len < target_len:
        # 补零
        pad_len = target_len - current_len
        signal = np.pad(signal, (0, pad_len), 'constant', constant_values=0)
        
    return torch.from_numpy(signal).unsqueeze(0) # [1, L]

def load_custom_signal(file_path, row_index=0):
    """
    加载外部文件 (.pkl, .npy)
    """
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"File not found: {file_path}")
        
    print(f"Loading signal from {file_path}...")
    
    if file_path.endswith('.pkl'):
        with open(file_path, 'rb') as f:
            content = pickle.load(f)
            # 假设结构是 {'data': np.array} 或直接是 np.array
            if isinstance(content, dict) and 'data' in content:
                data = content['data']
            else:
                data = content
    elif file_path.endswith('.npy'):
        data = np.load(file_path)
    else:
        raise ValueError("Unsupported file format. Please use .pkl or .npy")
        
    # 处理多维数据
    if data.ndim == 1:
        raw_signal = data
    elif data.ndim == 2:
        print(f"Data has shape {data.shape}, selecting row {row_index}")
        raw_signal = data[row_index]
    else:
        raise ValueError(f"Data shape {data.shape} is too complex.")
        
    return raw_signal

def run_inference(model, signal_tensor, device, save_path="recon_result.png"):
    """
    运行模型推理并绘图
    """
    signal_tensor = signal_tensor.to(device) # [1, L]
    
    print("Running inference...")
    with torch.no_grad():
        # 自动选择精度
        amp_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
        
        with autocast(device_type='cuda', dtype=amp_dtype):
            # 1. CWT 变换
            if signal_tensor.dim() == 3: signal_tensor = signal_tensor.squeeze(1)
            imgs = cwt_wrap(signal_tensor, num_scales=model.cwt_scales, lowest_scale=0.1, step=1.0)
            
            # 2. Instance Norm (模拟模型内部行为)
            imgs_f32 = imgs.float()
            mean = imgs_f32.mean(dim=(2, 3), keepdim=True)
            std = imgs_f32.std(dim=(2, 3), keepdim=True)
            std = torch.clamp(std, min=1e-5)
            imgs_norm = (imgs_f32 - mean) / std
            
            # 3. Encoder (获取 Mask 和 Latent)
            # 注意：这里 mask_ratio 使用模型配置的值 (如 0.75)
            latent, mask, ids_restore = model.forward_encoder(imgs_norm)
            
            # 4. Decoder
            pred_features = model.forward_decoder(latent, ids_restore)
            
            # 5. Heads
            pred_spec = model.decoder_pred_spec(pred_features)
            
            # 时域重建
            B, N, D = pred_features.shape
            H_grid, W_grid = model.grid_size
            feat_2d = pred_features.transpose(1, 2).view(B, D, H_grid, W_grid)
            feat_time_agg = model.time_reducer(feat_2d)
            feat_time_agg = feat_time_agg.squeeze(2).transpose(1, 2)
            pred_time = model.time_pred(feat_time_agg).flatten(1)

    # --- 绘图准备 ---
    orig_signal = signal_tensor[0].float().cpu().numpy()
    recon_signal = pred_time[0].float().cpu().numpy()
    
    # 简单反归一化用于显示
    orig_mean = orig_signal.mean()
    orig_std = orig_signal.std()
    recon_signal = recon_signal * (orig_std + 1e-6) + orig_mean

    # 频域处理
    p_h, p_w = model.patch_embed.patch_size
    B, C, H, W = imgs_norm.shape
    
    orig_spec = imgs_norm[0, 0, :, :].float().cpu().numpy()
    
    pred_patches = pred_spec[0].view(H // p_h, W // p_w, C, p_h, p_w)
    pred_patches = pred_patches.permute(2, 0, 3, 1, 4)
    recon_img = pred_patches.reshape(C, H, W)
    recon_spec = recon_img[0].float().cpu().numpy()
    
    # Mask 可视化
    mask_patch = mask[0].view(H // p_h, W // p_w)
    mask_pixel = mask_patch.repeat_interleave(p_h, dim=0).repeat_interleave(p_w, dim=1)
    mask_pixel = mask_pixel.float().cpu().numpy()
    
    masked_input_spec = orig_spec.copy()
    masked_input_spec[mask_pixel == 1] = np.min(orig_spec) 

    # --- 绘图 ---
    plt.figure(figsize=(16, 12))
    
    plt.subplot(4, 1, 1)
    plt.plot(orig_signal, label='Input Signal', color='black', alpha=0.7, linewidth=1)
    plt.plot(recon_signal, label='Reconstructed', color='red', alpha=0.7, linewidth=1)
    plt.title(f"Time Domain Reconstruction (Mask Ratio: {model.mask_ratio})")
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.subplot(4, 1, 2)
    plt.imshow(orig_spec, aspect='auto', origin='lower', cmap='jet')
    plt.title("Original CWT Spectrogram")
    plt.colorbar()

    plt.subplot(4, 1, 3)
    plt.imshow(masked_input_spec, aspect='auto', origin='lower', cmap='jet')
    plt.title("Masked Input (What Model Sees)")
    plt.colorbar()

    plt.subplot(4, 1, 4)
    plt.imshow(recon_spec, aspect='auto', origin='lower', cmap='jet')
    plt.title("Reconstructed CWT")
    plt.colorbar()

    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    print(f"Visualization saved to {save_path}")
    plt.close()

def main():
    parser = argparse.ArgumentParser(description="Visualize Reconstruction on New Signal")
    parser.add_argument('--config', type=str, default='config.yaml', help='Path to config file')
    parser.add_argument('--checkpoint', type=str, required=True, help='Path to model checkpoint')
    parser.add_argument('--input_file', type=str, required=True, help='Path to .pkl or .npy signal file')
    parser.add_argument('--row_index', type=int, default=0, help='Row index if file contains multiple signals')
    parser.add_argument('--output', type=str, default='recon_new_signal.png', help='Output image path')
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # 1. 加载模型
    model, config = load_model(args.config, args.checkpoint, device)

    # 2. 加载并预处理新信号
    raw_signal = load_custom_signal(args.input_file, args.row_index)
    signal_tensor = preprocess_signal(raw_signal, target_len=config['data']['signal_len'])
    
    # 3. 运行推理
    run_inference(model, signal_tensor, device, save_path=args.output)

if __name__ == "__main__":
    main()